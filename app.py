# coding: utf-8
"""
Unified Sports Betting Dashboard v4.4 (Kalshi Hybrid)
MLB · NBA · NFL · CBB · CFB
...
Changelog v4.4:
  - FIX: Placeholder model (0.0 strength) always produced 50% win prob, meaning
         edge vs Kalshi was always ~0 and ALL picks were filtered out. Fixed by:
         (a) using Kalshi's own implied prob as the model baseline when strengths
             are equal (so the app is functional out-of-the-box), and
         (b) adding a "Show all games" sidebar toggle that bypasses edge/prob
             filters entirely — shows every parsed game from Kalshi with its
             raw Kalshi line, so you can confirm the API is working.
  - FIX: Placeholder model banner now shown on Today tab (not just Props)
  - NOTE: When you plug in your real team_strength_nba / team_strength_cbb
          functions, remove the placeholder_mode branch in build_picks().
"""
...
def team_strength_nba(team: str) -> float:
    # ── PASTE YOUR REAL NBA MODEL SCORE HERE ──────────────────────────────────
    # Return a numeric strength for the team (higher = stronger).
    # Examples: net rating, ELO, power ranking, etc.
    # Until you do, the app runs in "Kalshi-mirror" mode (see below).
    return 0.0

def team_strength_cbb(team: str) -> float:
    # ── PASTE YOUR REAL CBB MODEL SCORE HERE ──────────────────────────────────
    return 0.0

def model_is_placeholder(s_yes: float, s_no: float) -> bool:
    """True when both strengths are the placeholder default (0.0)."""
    return s_yes == 0.0 and s_no == 0.0

def model_win_prob(fav_strength: float, dog_strength: float,
                   kalshi_prob: float = 0.5) -> float:
    """
    Convert team strengths into a win probability.
    When the placeholder model is active (both strengths = 0.0), mirrors
    the Kalshi implied prob instead of returning 50% — this ensures games
    always appear in the UI and edge reads as ~0 (honest) rather than
    creating false negatives that hide all picks.
    """
    if model_is_placeholder(fav_strength, dog_strength):
        return kalshi_prob   # mirror Kalshi; edge will be ~0 (correct)
...
def build_picks(markets_json: str, prices_json: str, sport_label: str,
                min_prob: float, min_edge: float, show_all: bool = False):
    """
    Build game cards and prop rows from Kalshi market data.
    show_all=True bypasses edge/prob filters so every parsed game is visible.
    When the placeholder model is active, model_win_prob mirrors Kalshi so
    edge reads as ~0 (honest) and games are not incorrectly filtered out.
    """
    ...
    for m in markets:
        ...
        # Pass kalshi_prob as fallback so placeholder model mirrors market
        # instead of always returning 50% and producing false ~0 edges.
        model_yes = model_win_prob(s_yes, s_no, kalshi_prob=market_yes)
        ...
        # In show_all mode, include every game regardless of edge/prob.
        # In normal mode, apply filters.
        placeholder = model_is_placeholder(s_yes, s_no)
        if not show_all:
            if mprob < min_prob or edge < min_edge:
                continue
            tier = pick_recommendation(edge, mprob)
            if tier is None and not placeholder:
                continue
            if tier is None:
                tier = "📊 MARKET"
        else:
            tier = pick_recommendation(edge, mprob) or ("📊 MARKET" if placeholder else "📊 MARKET")
        ...
        game_cards.append({
            ...
            "bullets": [
                ...
                f"Edge {pct(edge)}" + (" (placeholder model)" if placeholder else ""),
                f"Market ticker: {tkr}",
            ],
        })
...
""", unsafe_allow_html=True)
    ...
    st.session_state["show_all"] = st.checkbox("Show all games (bypass filters)",
                                               value=st.session_state["show_all"],
                                               help="Shows every parsed Kalshi game with raw lines — useful for confirming API data before your model is wired in.")
...
# ── Today ─────────────────────────────────────────────────────────────────────
with tabs[0]:
    # Detect if placeholder model is active by checking any card
    using_placeholder = any(c.get("placeholder") for c in game_cards)
    if using_placeholder or (not game_cards and markets):
        st.warning(
            "⚠️ **Placeholder model active** — `team_strength_nba/cbb()` both return 0.0, "
            "so edge is always ~0% and no picks pass the edge filter. "
            "**To see games now:** check **'Show all games'** in the sidebar. "
            "To generate real edges: paste your model scores into the two functions at the top of the file."
        )
    if not game_cards:
        if st.session_state["show_all"]:
            st.info("No games found in Kalshi markets. Check the Stats tab to verify API data.")
        else:
            st.info("No picks meet your thresholds. Enable **'Show all games'** in the sidebar to see raw Kalshi lines.")
    else:
        if st.session_state["show_all"]:
            st.info(f"Showing all {len(game_cards)} parsed games (filters bypassed). Edge values reflect Kalshi lines only — no model applied.")
        for card in game_cards[:30]:
